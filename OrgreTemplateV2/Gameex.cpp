...
public:
    ...
    class ExampleFrameListener(Ogre::SceneNode* node)
    {
        _node = node;
    }
...
bool OgreTutorial::keyPressed(const KeyboardEvent& evt)
{
    ...
    {
        if (SinbadNode->getPosition().x > 70)
        {
            SinbadNode->setPosition(68, SinbadNode->getPosition().y, SinbadNode->getPosition().z);
        }
        else
        {
            SinbadNode->translate(Ogre::Vector3(1.5, 0, 0));
        }
    }
...
void OgreTutorial::createScene()
{
 ...
 //
    ...
    groundEntity->setMaterialName("Examples/TextureEffect4");


    Entity* ent = scnMgr->createEntity("ground");
    ent->setCastShadows(true);
    SinbadNode = scnMgr->createSceneNode("Character");
    SinbadNode->attachObject(ent);
    scnMgr->getRootSceneNode()->addChild(SinbadNode);
    SinbadNode->setPosition(Ogre::Vector3(0.0f, 1.0f, 50.0f));
    SinbadNode->setScale(0.015f, 0.005f, 0.005f);
